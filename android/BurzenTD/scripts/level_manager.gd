...
func get_level_config() -> Dictionary:
	...
	var board_layout := _build_board_layout(normalized_level, rng)
	var map_mutation := _build_map_mutation(normalized_level, rng)
	var layout_profile := _build_layout_profile(normalized_level, rng, board_layout.get("wall_density", 0.2))
	var wave_definition := _build_wave_definition(normalized_level, rng)
	var tower_nodes := board_layout.get("tower_nodes", [])
...
func _build_board_layout(next_level_index: int, rng: RandomNumberGenerator) -> Dictionary:
	var wall_density := clamp(rng.randf_range(WALL_MIN_DENSITY, WALL_MAX_DENSITY) + float(next_level_index) * 0.00005, WALL_MIN_DENSITY, WALL_MAX_DENSITY)
	var spawn_tile := _pick_edge_tile(rng, true)
	var exit_tile := _pick_edge_tile(rng, false)
	var path_tiles := _carve_seed_path(spawn_tile, exit_tile, rng)
	var path_set := {}
	...
	var tower_nodes := _build_tower_nodes_from_tiles(path_set, walls, rng)
	return {
		"wall_density": wall_density,
...
func _carve_seed_path(spawn_tile: Vector2i, exit_tile: Vector2i, rng: RandomNumberGenerator) -> Array[Vector2i]:
	...
	while current != exit_tile:
		...
		if rng.randf() < 0.25:
			var lateral := [Vector2i.LEFT, Vector2i.RIGHT, Vector2i.UP, Vector2i.DOWN]
			lateral.shuffle()
			for move in lateral:
